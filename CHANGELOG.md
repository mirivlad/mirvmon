...
- Replaced the global CPU/RAM median detector with deterministic `level_shift_v2` contextual baselines built from up to 56 days of hourly history in `APP_TIMEZONE`: same weekday/hour first, then weekday-vs-weekend/hour, then hour-of-day.
...
- Added explicit level-shift hysteresis: trigger disappearance is `elevated`, configured warning ownership is `incident_owned`, and an open anomaly resolves only after twelve consecutive normal 5-minute buckets using the recovery boundary captured at episode start.
