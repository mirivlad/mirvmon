...
$ModernArtifact = 'windows-amd64'
$ModernSha256 = '__MODERN_SHA256__'
$ModernSize = __MODERN_SIZE__
$LegacyArtifact = 'windows-legacy-amd64'
$LegacySha256 = '__LEGACY_SHA256__'
$LegacySize = __LEGACY_SIZE__
$ExpectedArtifact = ''
$ExpectedSha256 = ''
$ExpectedSize = 0
...
$PackageModernAgentPath = Join-Path $InstallerDir 'mirvmon-agent-modern.exe'
...
$StageBootstrapPath = Join-Path $StageDir 'bootstrap.json'
...
function Select-AgentArtifact {
    $OperatingSystems = @(Get-WmiObject -Class Win32_OperatingSystem -ErrorAction Stop)
    if ($OperatingSystems.Count -ne 1) {
        throw 'prerequisites: Windows version is unavailable.'
    }
    $OperatingSystem = $OperatingSystems[0]
    $IsX64 = ($OperatingSystem.OSArchitecture -match '64') -and
        (($env:PROCESSOR_ARCHITECTURE -eq 'AMD64') -or ($env:PROCESSOR_ARCHITEW6432 -eq 'AMD64'))
    if (-not $IsX64) {
        throw 'prerequisites: only x64 Windows is supported.'
    }
    $WindowsVersion = New-Object Version -ArgumentList ([string] $OperatingSystem.Version)
    if (($WindowsVersion.Major -eq 6) -and ($WindowsVersion.Minor -eq 1)) {
        if ([int] $OperatingSystem.ServicePackMajorVersion -lt 1) {
            throw 'prerequisites: Windows 7 and Server 2008 R2 require Service Pack 1.'
        }
        $script:PackageAgentPath = $PackageLegacyAgentPath
        $script:ExpectedArtifact = 'windows-legacy-amd64'
        $script:ExpectedSha256 = $LegacySha256
        $script:ExpectedSize = $LegacySize
        return
    }
    if (($WindowsVersion.Major -eq 6) -and (($WindowsVersion.Minor -eq 2) -or ($WindowsVersion.Minor -eq 3))) {
        $script:PackageAgentPath = $PackageLegacyAgentPath
        $script:ExpectedArtifact = 'windows-legacy-amd64'
        $script:ExpectedSha256 = $LegacySha256
        $script:ExpectedSize = $LegacySize
    ...
    if ($WindowsVersion.Major -ge 10) {
        $script:PackageAgentPath = $PackageModernAgentPath
        $script:ExpectedArtifact = 'windows-amd64'
        $script:ExpectedSha256 = $ModernSha256
        $script:ExpectedSize = $ModernSize
        return
    }
    throw 'prerequisites: unsupported Windows version.'
}
...
try {
    ...
    if (-not $Principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        throw 'prerequisites: run MirvMon-Agent-Setup.exe as Administrator.'
    }
    Select-AgentArtifact
    if (-not (Test-Path -LiteralPath $PackageAgentPath -PathType Leaf)) {
        throw 'prerequisites: bundled mirvmon-agent.exe is missing.'
    }
    if (-not (Test-Path -LiteralPath $PackageBootstrapPath -PathType Leaf)) {
        throw 'prerequisites: bundled bootstrap.json is missing.'
    }
    ...
    Copy-Item -LiteralPath $PackageBootstrapPath -Destination $StageBootstrapPath
    ...
    Write-Stage 'Activating this installation.'
