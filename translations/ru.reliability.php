...
return [
    ...
    'reliability.method' => 'Для серверов доступность рассчитана по переходам online/offline и показывается при полноте данных не ниже 95%. Для сайтов доступность рассчитана по успешным автоматическим проверкам основного endpoint, включая проверки содержимого, и показывается по имеющимся данным; при полноте ниже 95% результат помечается как неполный период. Полнота — отношение полученных измерений к ожидаемым при текущем интервале. Инциденты могут пересекаться, поэтому их суммарная длительность не равна времени простоя.',
];
