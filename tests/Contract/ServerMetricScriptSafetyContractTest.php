...
final class ServerMetricScriptSafetyContractTest extends TestCase
{
    ...
    public function testLiveMetricRefreshPreservesTimestampViewport(): void
    {
        ...
        self::assertStringContainsString('new Date(oldStart + shift).toISOString()', $script);
        self::assertStringContainsString('new Date(oldEnd + shift).toISOString()', $script);
        self::assertStringContainsString('timestampIndex(timestamps, shifted.start, 0)', $script);
        self::assertStringNotContainsString('metricViewportState', $script);
        self::assertStringNotContainsString('metricZoomTriggers', $script);
