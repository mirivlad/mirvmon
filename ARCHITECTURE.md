...
CPU/RAM detector сравнивает свежие 5-minute buckets с robust p10/median/p90 baseline из hourly aggregates. Disk detector перед анализом объединяет mount aliases одного filesystem по совпадающим `disk_total_gb_*`, current usage и hourly history, затем берёт canonical `disk_used_*`, отбрасывает историю до последней заметной очистки и оценивает текущий линейный trend с quality gate. Оба детектора возвращают versioned fingerprint и evidence, достаточный для UI/notification без повторного вычисления объяснения.

Lifecycle использует `active`, `handled`, `accepted_normal`, `resolved`, но identity различается по типу observation. Prediction остаётся fingerprint-driven. Для anomaly fingerprint описывает обучаемый pattern, а текущий episode определяется open row для `server + metric + detector`: изменение value band обновляет ту же row и не создаёт новое уведомление. `handled` означает проверенный текущий episode и не обучает normal pattern; после recovery следующий episode создаётся отдельно и снова может уведомить. `accepted_normal` остаётся явным suppression конкретного fingerprint pattern до reset. Auto-resolve разрешён только для метрики, которую текущий worker-run действительно смог оценить; maintenance/DR/data gap не считается recovery. Observation outbox jobs используют `observation_id` и существующий recipient resolution, не создавая fake alert row. Formatter при наличии `PUBLIC_BASE_URL` добавляет direct server link и, для observation, отдельный observation anchor.
