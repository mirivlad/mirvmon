...
CPU/RAM `level_shift_v2` сравнивает свежие 5-minute buckets с contextual baseline из последних 56 дней hourly aggregates: lower envelope строится по историческим hourly minima, median — по hourly averages, upper envelope — по hourly maxima, чтобы регулярная внутридневная волатильность не исчезала из модели после усреднения. Контекст вычисляется в `APP_TIMEZONE`: тот же weekday и час с окном ±1 имеет приоритет, затем weekday/weekend + час, затем hour-of-day; одной общей медианы недостаточно для создания anomaly. Если contextual history ещё не созрела, detector остаётся невооружённым. Disk detector перед анализом объединяет mount aliases одного filesystem по совпадающим `disk_total_gb_*`, current usage и hourly history, затем берёт canonical `disk_used_*`, отбрасывает историю до последней заметной очистки и оценивает текущий линейный trend с quality gate. Детекторы возвращают versioned fingerprint и explainable evidence, достаточный для UI/notification без повторного вычисления объяснения.

Lifecycle использует `active`, `handled`, `accepted_normal`, `resolved`, но identity различается по типу observation. Prediction остаётся fingerprint-driven. Для anomaly fingerprint описывает обучаемый pattern, а текущий episode определяется open row для `server + metric + detector`: изменение value band обновляет ту же row и не создаёт новое уведомление. `handled` означает проверенный текущий episode и не обучает normal pattern; после recovery следующий episode создаётся отдельно и снова может уведомить. Для `level_shift_v2` generic missing-grace не используется: состояние `elevated` сохраняет episode, `incident_owned` передаёт приоритет обычному alert pipeline без закрытия observation, а `clear` требует часа последовательных 5-minute buckets ниже recovery boundary текущего contextual baseline. Это позволяет плановой смене ночного/дневного режима завершить старый episode только после устойчивого часа, а не мгновенно на границе контекста. `accepted_normal` для v2 сопоставляется по weekday/hour и ограниченному value range; maintenance/DR/data gap не считается recovery. Observation outbox jobs используют `observation_id` и существующий recipient resolution, не создавая fake alert row. Formatter при наличии `PUBLIC_BASE_URL` добавляет direct server link и, для observation, отдельный observation anchor.
...
Config poll дополнительно устраняет устаревшую активную команду: repository под
row lock переводит старый `pending` в `failed/target_superseded`, но не создаёт
новый UUID автоматически. Агент мог сохранить local `accepted` до неуспешного
HTTP-report, поэтому безопасная новая команда создаётся только явным повтором
администратора после local cleanup. Состояния `accepted` и далее сервер не
меняет, потому что обновление уже могло начаться на наблюдаемом узле.
