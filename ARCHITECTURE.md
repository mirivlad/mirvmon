...
CPU/RAM `level_shift_v2` сравнивает свежие 5-minute buckets с robust p10/median/p90 contextual baseline из последних 56 дней hourly aggregates. Контекст вычисляется в `APP_TIMEZONE`: тот же weekday и час с окном ±1 имеет приоритет, затем weekday/weekend + час, затем hour-of-day; одной общей медианы недостаточно для создания anomaly. Если contextual history ещё не созрела, detector остаётся невооружённым. Disk detector перед анализом объединяет mount aliases одного filesystem по совпадающим `disk_total_gb_*`, current usage и hourly history, затем берёт canonical `disk_used_*`, отбрасывает историю до последней заметной очистки и оценивает текущий линейный trend с quality gate. Детекторы возвращают versioned fingerprint и explainable evidence, достаточный для UI/notification без повторного вычисления объяснения.
