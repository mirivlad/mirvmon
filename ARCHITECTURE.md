...
Lifecycle использует `active`, `handled`, `accepted_normal`, `resolved`, но identity различается по типу observation. Prediction остаётся fingerprint-driven. Для anomaly fingerprint описывает обучаемый pattern, а текущий episode определяется open row для `server + metric + detector`: изменение value band обновляет ту же row и не создаёт новое уведомление. `handled` означает проверенный текущий episode и не обучает normal pattern; после recovery следующий episode создаётся отдельно и снова может уведомить. Для `level_shift_v2` generic missing-grace не используется: состояние `elevated` сохраняет episode, `incident_owned` передаёт приоритет обычному alert pipeline без закрытия observation, а `clear` требует часа последовательных 5-minute buckets ниже recovery boundary, сохранённой при открытии episode. `accepted_normal` для v2 сопоставляется по weekday/hour и ограниченному value range; maintenance/DR/data gap не считается recovery. Observation outbox jobs используют `observation_id` и существующий recipient resolution, не создавая fake alert row. Formatter при наличии `PUBLIC_BASE_URL` добавляет direct server link и, для observation, отдельный observation anchor.
