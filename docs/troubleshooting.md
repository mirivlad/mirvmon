...
Счётчики последовательных результатов ограничены порогами перехода состояния:
три ошибки или два успеха. Если worker пишет
`website_check_worker_database_unavailable`, сопоставьте время сообщения с
журналом PostgreSQL: этот маркер также появляется при ошибке SQL-запроса,
когда сама БД доступна.

На `/admin/system` также показаны задания проверки сайтов: ожидающие,
обрабатываемые, просроченные и исчерпавшие попытки. После десятой неудачной
попытки задание переходит в `failed` и больше не мешает ручному запуску новой
проверки. Если worker завершился во время последней попытки, он переведёт
задание в `failed` после истечения аренды. Число последних отказов влияет на
статус системы только в течение часа; общий счётчик сохраняет историю.
