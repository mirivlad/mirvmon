...
- Up to 56 days of hourly history are interpreted in `APP_TIMEZONE`; the worker prefers the same weekday/hour (±1h), then weekday-vs-weekend/hour, then hour-of-day. No contextual history means no anomaly rather than a global fallback.
...
- Recovery keeps the boundary captured when the episode opened, preventing context changes from ending an episode.
